<?php session_start();
	...
	if(empty($email) || empty($email) || empty($email) || !isset($_SESSION['change']) )
	{
   	...
   	if(isset($_POST['username']) && isset($_POST['password']) && isset($_POST['c_password']) )
   	{
   		...
   		{	
   			if($user == $row['username'])
   			{
				...
				$query="UPDATE login SET  password='$pass', expire='$expire' WHERE username='$user' AND otp='$token' ";
				...
				$message .= "You've successfully changed your Blog-Me password.\r\n";
				$message .= "Thanks for using Blog-Me\r\n";
				$message .= "The Blog-Me Team\r\n";
...
<?php include 'includes/alert.inc.php'; ?>
  ...
  <div class="container">
    <div class="info">
      <h2 style="color: white;">Reset Password</h2><span>
    </div>
  </div>
 ...
 <?php echo '<form class="forgot-form" action="change?ee='.$email.'&tok='.$token.'&em='.$mailu.'&'.$email.'" method="POST" >'; ?>
    
    <p class="message" style="text-align:left;">Username</p>
    <input type="text" placeholder="username" name="username" required />
    
    <p class="message" style="text-align:left;">New Password</p>
    <input type="password" placeholder="New password.." name="password" required />
    
    <p class="message" style="text-align:left;">Confirm Password</p>
